from src.tokenizer import Tokenizer, KeywordToken, NumberToken, StringToken, IdentifierToken, SymbolToken
...
class Parser:
    ...
    def parse(self):
        ...
        if self.accept(KeywordToken('nil')):
            self.next()
            return NilValue()
        elif self.accept_type(NumberToken):
            return IntegerValue(self.next().value)
        elif self.accept_type(StringToken):
            return StringValue(self.next().value)
        elif self.accept_type(IdentifierToken):
            if self.accept(SymbolToken('[')):
                return self.array()
    ...
    def array(self):
        ...
        return ArrayCreation(id, exp1, ex
